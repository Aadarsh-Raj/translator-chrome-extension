...
<p style="letter-spacing:1px; font-size:1.5rem; word-wrap:break-word"> Please select words </p>
</div>`;
...
<p style="letter-spacing:1px; font-size:1.5rem; word-wrap:break-word"> ${text} </p>
</div>`;
};
...
function getSectedText() {
  ...
  if (isDragging) {
    // when container is dragged, it will not select the text
    return;
  }
  let text = "";
  if (document.getSelection && !container.getSelection) {
...
<p style="letter-spacing:1px; font-size:1.5rem; word-wrap:break-word"> Something went wrong,Please try again letter </p>
</div>`;
  }
};

// added debouncing to stop unnecessary calling of api
